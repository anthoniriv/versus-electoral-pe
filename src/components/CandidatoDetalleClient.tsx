    ...
    <div className="min-h-screen bg-gray-950">
      <section className="py-12 px-4">
        <div className="mx-auto max-w-4xl">
          {/* Breadcrumb */}
          <nav className="mb-8 text-sm text-gray-500 flex items-center gap-1.5 flex-wrap">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 mb-8">
            <CandidatoAvatar
              ...
              nombre={nombre}
              size={112}
              className="rounded-xl border-4 shrink-0"
              style={{
                borderColor: GRAVEDAD[peorGravedad].color,
                boxShadow: `0 0 25px ${GRAVEDAD[peorGravedad].color}30`,
              }}
            />
            <div className="text-center sm:text-left">
              <h1 className="text-3xl font-black text-white">{nombre}</h1>
              <p className="mt-1 text-lg text-gray-400">{partido}</p>
          ...
          <div className="mb-6">
            <input
              ...
              placeholder="Buscar en noticias de este candidato..."
              className="w-full rounded-xl bg-gray-900/60 border border-gray-800/60 text-white text-sm px-4 py-3 placeholder-gray-500 focus:outline-none focus:border-gray-600 transition-colors"
            />
          </div>

          {/* Gravedad breakdown */}
          {Object.keys(gravedadCounts).length > 0 && (
            <div className="mb-10 grid grid-cols-2 sm:grid-cols-5 gap-2.5">
              {ORDEN_GRAVEDAD.map((g) => {
                ...
                return (
                  <button
                    ...
                    className={`group rounded-xl border bg-gray-900/50 p-3.5 text-center transition-all duration-200 ${
                      active
                  ...
                  >
                    <p className="text-2xl font-black transition-transform duration-200 group-hover:scale-110" style={{ color: GRAVEDAD[g].color }}>
                      {gravedadCounts[g] || 0}
                    </p>
                    <p className="text-[11px] text-gray-500 mt-0.5">{GRAVEDAD[g].label}</p>
                  </button>
                );
              })}
            </div>
